...
class TextFormPage extends StatefulWidget {
  const TextFormPage({super.key, required this.bloc});
  final AuthBloc bloc;
  @override
...
class _TextFormPageState extends State<TextFormPage> {
  ...
  @override
  void dispose() {
    widget.bloc.nameController.dispose();
    widget.bloc.emailController.dispose();
    widget.bloc.passwordController.dispose();
    super.dispose();
  }
      ...
      child: Column(
        children: [
          CustomTextFromGlobal(
            ...
            prefixIcon: Icon(Icons.person, color: ColorApp.primery),
            validator: (value) {
              if (keyRegister.currentState!.validate()) {
                return validationField("text", 3, 50, value);
              }
              return null;
            },
          ),
          ...
          CustomTextFromGlobal(
            ...
            validator: (value) {
              if (keyRegister.currentState!.validate()) {
                return validationField("email", 10, 100, value);
              }
              return null;
            },
          ),
